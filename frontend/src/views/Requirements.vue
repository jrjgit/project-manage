    ...
    <div class="requirements-page">
      ...
      <section class="section-card">
        <div class="filter-bar">
          ...
          <n-select v-model:value="filters.assignee_id" :options="userOptions" placeholder="开发人员" clearable filterable style="width:140px" size="small" />
...
const filters = ref({ number: '', description: '', status: null, project_type: null, project_id: null, iteration_id: null, system: null, assignee_id: null, overdue: false })
...
function resetFilters() {
  filters.value = { number: '', description: '', status: null, project_type: null, project_id: null, iteration_id: null, system: null, assignee_id: null, overdue: false }
}
...
function buildFilterQuery() {
  ...
  if (f.assignee_id) q.assignee_id = f.assignee_id
...
function restoreFiltersFromQuery() {
  ...
  filters.value = {
    ...
    assignee_id: q.assignee_id ? Number(q.assignee_id) : null,
...
async function loadData() {
  try {
    ...
    if (f.assignee_id) params.assigneeId = f.assignee_id
...
watch(
  () => {
    const f = filters.value
    return [f.status, f.system, f.project_type, f.project_id, f.iteration_id, f.overdue, f.number, f.description, f.assignee_id]
  },
  () => {
