...
import { getSystems, createSystem, updateSystem, deleteSystem } from '@/api/systems'
import AppLayout from '@/components/AppLayout.vue'
import { NButton, NModal, NForm, NFormItem, NInput, NSpace, NDataTable, NTag } from 'naive-ui'
...
async function submit() {
  if (!form.value.name.trim()) { window.$message?.warning('请输入系统名称'); return }
  submitting.value = true
  try {
    if (editingId.value) {
      await updateSystem(editingId.value, form.value)
