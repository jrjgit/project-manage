<template>
  <div class="task-card" v-bind="$attrs">
    ...
    <div class="card-body">
      ...
      <div v-if="task.deadline" class="card-deadline" :class="{ overdue: overdueDays > 0 }">
...
const overdueDays = computed(() => {
  if (!props.task.deadline || props.task.status === 'closed') return 0
  const now = new Date()
  const end = new Date(props.task.deadline)
...
.card-title { font-size: 13px; font-weight: 600; color: #1e293b; margin-bottom: 8px; line-height: 1.5; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; }
...
.card-deadline { font-size: 11px; color: #64748b; margin-top: 8px; display: flex; align-items: center; gap: 4px; }
.card-deadline svg { width: 12px; height: 12px; }
.card-deadline.overdue { color: #d03050; font-weight: 600; }
